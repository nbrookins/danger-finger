...
- **2026-03** – Web UI fixes: added `<meta charset="utf-8">` to prevent title/h1 mojibake; replaced hardcoded 4-button part visibility row with dynamic buttons auto-populated from `/api/parts` so all 9 parts (tip, middle, base, linkage, tipcover, socket, plug, stand, pins) are toggleable. Extended `scripts/inspect_ui.py` with checks for title encoding, part toggle completeness, plugInstances in previewConfig, advanced section collapse, and explode slider state. Updated `BUILD_NUMBER` default in Makefile to include git short SHA so Docker image tags are traceable to a specific commit.
