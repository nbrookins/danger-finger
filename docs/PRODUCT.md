...
| Decision | Rationale |
|----------|-----------|
| **Async preview via job queue** | Preview now enqueued at PRIORITY_PREVIEW=-10 (always first) in the shared render queue, results stored in-memory (pruned after 10 min), frontend polls via /jobs/{id}. Eliminates the 10s timeout error for slow previews. |
...
| **Parameters: Common → Standard → Advanced sections** | Most-used sizing params (6 tagged `section="common"`) appear first in a highlighted "Common" section; standard params follow; advanced params remain collapsed. Deterministic ordering via `Prop.order` and `Prop.section`. |
| **Preset configurations** | Dropdown above Common section with 4 presets (Adult index, Adult pinky, Child index, Adult thumb). Applies common param values and triggers preview. |
| **Per-param and global reset** | Each modified row shows an × button to reset that param to default; "Reset all to defaults" link resets everything. |
| **Render button intelligence** | Render button tracks `_lastRenderedCfghash`; disabled when current config matches last render; re-enabled on param change. Labels: Render / Rendering... / Queued... / Rendered. |
| **Full-page launch and share links** | "Open full page" button shown in iframe context; "Share link" button encodes params in URL hash, copies to clipboard. URL hash restored on load. |
| **Profile table status** | Status column shows Ready/Rendering.../Queued.../Saved per config, with polling for in-progress renders. |
| **Quality indicator** | Preview status shows "(draft quality)" or "(print quality)" based on whether current config has been rendered. |
...
- **2026-03** – UX overhaul: async preview queue (preview jobs processed at highest priority, polled by frontend), deterministic parameter ordering with Common/Standard/Advanced sections, render button intelligence (disabled when config matches last render), iframe full-page launch and shareable config links via URL hash, profile table render status column (Saved/Rendering/Ready), per-parameter and global reset-to-defaults, draft vs print quality indicator in viewer status, and preset configurations dropdown (Adult index, Child index, Adult pinky, Adult thumb). New Prop attributes: `order` and `section` for deterministic param display.
- **2026-03** – Anonymous access, authenticated save, durable render queue: WordPress now embeds the configurator for guests, while login is only required to save to a profile. Full renders submit through a durable S3-backed job queue with authenticated-first priority, guest throttling, job polling by `job_id` or `cfghash`, and restart recovery that requeues interrupted work after watchdog/container restarts. Frontend save and render are now separate actions, guest draft state survives the WordPress login redirect, and completed bundles remain downloadable without re-rendering.
- **2026-03** – Monitoring and auto-recovery: added CloudWatch-based EC2 system recovery, a scheduled health-check Lambda that probes `GET /api/parts`, automatic SSM container restart attempts, SNS email alerting, and operator commands for checking health/alarm state.
...
| `geometry-validation.mdc` | `finger.py`, `finger_base.py`, `finger_params.py` | Topology checks, `_fn` visibility rule, post-CSG-rewrite checklist |
| `python-to-scad-translation.mdc` | `*.scad` | Transform order, formula, and OpenSCAD constraint pitfalls |
...
| `project-routing-map.mdc` | Always | Task-domain routing map — which docs to read for which task |
| `endpoint-state-map.mdc` | Always | Endpoint auth requirements, state locations, multi-instance implications |
