...
- **`_preview_hidden`**: Set of part names (e.g. `{"stand"}`) hidden by default in the preview UI.
...
- **`_preview_position_override`**: Optional overrides on top of `_preview_position_offsets` for a single part (viewer only).
